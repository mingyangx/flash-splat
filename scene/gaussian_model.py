...
class GaussianModel:
    ...
    def load_ply(self, path):
        ...
        # print("Loading ", colored(path, 'red'))
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, iter, freeze=False):
        
        if not freeze:
            if len(self._xyz) < 0.5e6 and self.get_features.size()[0] < 0.5e6:
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter, freeze=False):

        # if freeze:
        #     return

        if len(self._xyz) > 0.5e6 or self.get_features.size()[0] > 0.5e6:
            return
