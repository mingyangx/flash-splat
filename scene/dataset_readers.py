...
class CameraInfoPaired(NamedTuple):
    campaired: bool
    uid: int
    R: np.array
    T: np.array
    ...
    # CHM: added to handle flash images
    image_noflash: np.array
    # CHM: added to handle flash images
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, path, rgb_mapping, use_raw):
    cam_infos = []
    for idx, key in enumerate(sorted(cam_extrinsics.keys())):
        ...
        data_paired = True if '/images_noflash/' in path else False
        if not data_paired:
            if not use_raw:
                cam_info = CameraInfo(campaired=data_paired, uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_path=image_path,
                        image_name=image_name, width=width, height=height, mask=None, bounds=bounds, use_raw=use_raw)
            else:
                image_npy_path = image_path.replace("/images/", "/images_npy/").replace(".png", ".npy")
                image_npy = np.load(image_npy_path)
                cam_info = CameraInfo(campaired=data_paired, uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image_npy, image_path=image_npy_path,
                        image_name=image_name, width=width, height=height, mask=None, bounds=bounds, use_raw=use_raw)
            
        else:
            image_noflash_path = image_path.replace("/images_flash/", "/images_noflash/")
            image_noflash = Image.open(image_noflash_path)
            cam_info = CameraInfoPaired(campaired=data_paired, uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image, image_noflash=image_noflash, image_path=image_path,
...
from scene.fetch_pc import fetch_pc_main
'''Auto Cut'''
def fetchPly(args, which_scene=None):
    fetched_pc = fetch_pc_main(args.ply_pth_flash, args.ply_pth_noflash, args.ply_pth_mix, which_scene)
    positions = np.vstack([fetched_pc[:, 0], fetched_pc[:, 1], fetched_pc[:, 2]]).T
    colors = np.vstack([fetched_pc[:, 3], fetched_pc[:, 4], fetched_pc[:, 5]]).T / 255.0
    normals = np.vstack([fetched_pc[:, 6], fetched_pc[:, 7], fetched_pc[:, 8]]).T
...
def readColmapSceneInfo(args, llffhold=7):
    path = args.source_path
    images = args.images
    eval = args.eval
    n_views = args.n_views
    use_raw = args.use_raw
    args.ply_path = args.ply_pth_flash
    ply_path = args.ply_path
    which_scene = args.which_scene
    # ply_path = os.path.join(path, "sparse/0/points3D.ply")
    # bin_path = os.path.join(path, "sparse/0/points3D.bin")
    if ply_path == 'original':
        ply_path = os.path.join(path, str(n_views) + "_views/dense/fused.ply")
    else:
        ply_path = os.path.join(ply_path)
    ...
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics,
                             images_folder=os.path.join(path, reading_dir),  path=path, rgb_mapping=rgb_mapping, use_raw=use_raw)
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames), disable=True):
